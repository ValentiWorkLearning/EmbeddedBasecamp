...
void draw_current_time()
{
	...
	snprintf(conversion_buffer, sizeof(conversion_buffer), "%d:%d:%d",
		 loc_time->tm_hour, loc_time->tm_min, loc_time->tm_sec);

	const int x0_coord = LCD_WIDTH / 4;
	const int y0_counter = 40;
	lcd_put_string(conversion_buffer, x0_coord, y0_counter, Font_11x18,
...
void draw_ip_address()
{
	...
	lcd_put_string(local_ip, x0_coord, y0_counter, Font_11x18, COLOR_RED,
		       COLOR_BLACK);
}
...
void draw_counter_value()
{
	if (drawing_manager.counter_rect_invalidated) {
		...
		snprintf(conversion_buffer, sizeof(conversion_buffer), "%d",
			 counter_value);
		lcd_put_string(conversion_buffer, x0_counter, y0_counter,
			       Font_16x26, COLOR_RED, COLOR_BLACK);
		lcd_update_screen();
