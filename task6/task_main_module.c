...
static char *device_memory_buffer;
static int chardev_buffer_rw_holder = 0;

static ssize_t messenger_chardev_read(struct file *file, char *buf, size_t count, loff_t *ppos)
{
    ...
    int ret = copy_to_user(buf, device_memory_buffer, count);
    if (ret)
    {
...
static ssize_t messenger_chardev_write(struct file *filep, const char *buffer, size_t len, loff_t *offset)
{
    int ret;
    ...
    ret = copy_from_user(device_memory_buffer, buffer, chardev_buffer_rw_holder);
    if (ret)
    {
...
static int __init chardev_init(void)
{
    int ret;
    dev_t device;
    ret = alloc_chrdev_region(&device, MIN_MINOR_NUMBER, MAX_DEVICE_COUNT, MODULE_TITLE);
    ...
    ret = cdev_add(&hcdev, MKDEV(device_major_number, MIN_MINOR_NUMBER), MAX_DEVICE_COUNT);
    if (ret < 0)
    {
        unregister_chrdev_region(MKDEV(device_major_number, MIN_MINOR_NUMBER), MAX_DEVICE_COUNT);
