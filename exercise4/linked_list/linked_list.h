...
extern bool __list_add_valid(struct list_head* pnew,
	struct list_head* prev,
	struct list_head* next);
extern bool __list_del_entry_valid(struct list_head* entry);
#else
static inline bool __list_add_valid(struct list_head* pnew,
	struct list_head* prev,
	struct list_head* next)
{
	return true;
...
static inline void __list_add(struct list_head* pnew,
	...
	struct list_head* next)
{
	if (!__list_add_valid(pnew, prev, next))
		return;

	next->prev = pnew;
	pnew->next = next;
	pnew->prev = prev;
	WRITE_ONCE(prev->next, pnew);
}
...
static inline void list_del(struct list_head* entry)
{
	...
	entry->next = REINTERPRET_CAST(list_head*)(LIST_POISON1);
 ...
 * @list: The desired pnew front of the list.
 * @head: The head of the list.
 *
 * Rotates list so that @list becomes the pnew front of the list.
 */
