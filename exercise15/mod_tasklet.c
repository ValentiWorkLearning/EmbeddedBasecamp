...
void tasklet_function( struct tasklet_struct* htasklet ) { /* Bottom Half Function */
   context = in_atomic();
   j2 = jiffies;
   cycles2 = get_cycles();
   printk( "%010lld [%05d] : %s in contxt %d\n", 
           (long long unsigned)cycles2, j2, (char*)htasklet->data, context );
   return;
}
...
int init_module( void ) {
    my_tasklet.data = (unsigned int)(&tasklet_data);
   context = in_atomic();
   j1 = jiffies;
   cycles1 = get_cycles();
